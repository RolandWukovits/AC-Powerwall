...
// Version 1.00.30 (complete edition), 26.09.2020
...
// Current module
int currentcounter;
float maxCurrent=32768;
float minCurrent=32768;
float diffCurrent;
float tempdiffCurrent;
float readCurrent;
int innercounter;
// Blynk
byte blynkavail=0;
...
if (inverter==1){
inverterlog=((invcurrsens/65336)*diffCurrent)*230;      // calculate inverter watts
}
else{
inverterlog=0;  
}
...
void BlynkDataTransmit(){
 ...
 Blynk.virtualWrite(V8, round(realpower));
...
if (typeofbatt==1){
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.058*0.3);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.048*0.3);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.039*0.3);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.030*0.3);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.024*0.3);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.021*0.3);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.017*0.3);  
  }
  }
...
if (inverter==1) {         //correcting by inverter voltage by Current module
  SOCvoltage=SOCvoltage+(diffCurrent*0.0000109*(invcurrsens/5));
  tempdiffCurrent=diffCurrent*(invcurrsens/5);
