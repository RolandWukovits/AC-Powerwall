...
// Version 1.00.48 (complete edition), 27.10.2020
...
byte EEPROMvalue[40];         //Array accomodating 40 stored values, increase if needed
//LED announciators
long ledcounter=0;
byte LEDgreen;
byte LEDred;
...
void setup()
{
  ...
  if (EEPROMvalue[0]==255 or EEPROMvalue[0]==0) {
     ...
     EEPROM.write(31,dyntrigger);               // spare slot
     EEPROM.write(32,1);
     EEPROM.write(33,1);
     EEPROM.write(34,1);
     EEPROM.write(35,1);
...
ledcounter++;
 ...
 if (mode==9 and loopcounter>=4000){
  Mode9();
  loopcounter=0;
  wdt_reset();
 }

 if (mode==10 and loopcounter>=4000){
  Mode10();
  loopcounter=0;
  wdt_reset();
 }

 if (mode==11 and loopcounter>=4000){
  Mode11();
  loopcounter=0;
  wdt_reset();
 }
 
 if (mode==12 and loopcounter>=4000){
  Mode12();
  loopcounter=0;
  wdt_reset();
 }
 ...
 if (mode<9){ 
  if (currentcounter==5 or currentcounter==15) {
 ...
 if (currentcounter==1 and ledcounter<=75){
  Currentaquisition();
 }
...
if (typeofbatt==1 and inverter==1) {
  chargerlock=0;
   ...
   if (realpower>(((dynlevel+triggerpowermode1)*10)+(52*ConeAmps)) and realpower<(((dynlevel+triggerpowermode1)*10)+(52*CtwoAmps))) {
     digitalWrite(relayPin1,LOW);
     digitalWrite(relayPin2,HIGH);
     chargerone=1;
     chargertwo=0;
   }
    if (realpower>=(((dynlevel+triggerpowermode1)*10)+(52*CtwoAmps)) and realpower<(((dynlevel+triggerpowermode1)*10)+(52*ConeAmps)+(52*CtwoAmps))) {
     digitalWrite(relayPin1,HIGH);
     chargerone=0;
     digitalWrite(relayPin2,LOW);
     chargertwo=1;
   } 
   if (realpower>=(((dynlevel+triggerpowermode1)*10)+(52*ConeAmps)+(52*CtwoAmps))) {
     digitalWrite(relayPin1,LOW);
     chargerone=1;
     if (chargerlock==0) {
     digitalWrite(relayPin2,LOW);
...
if (battSOC<=maximumSOC and Maxlock==0) {
  ...
  chargerlock=0;
   ...
   if (realpower<=((dynlevel*10+(triggerpowermode2*-10))+(52*ConeAmps))) {
     digitalWrite(relayPin1,HIGH);
     digitalWrite(relayPin2,HIGH);
     chargerone=0;
     chargertwo=0;
   } 
   if (realpower>((dynlevel*10+(triggerpowermode2*-10))+(52*ConeAmps)) and realpower<((dynlevel*10+(triggerpowermode2*-10))+(52*CtwoAmps))) {
     digitalWrite(relayPin1,LOW);
     digitalWrite(relayPin2,HIGH);
     chargerone=1;
     chargertwo=0;
   }
    if (realpower>=((dynlevel*10+(triggerpowermode2*-10))+(52*CtwoAmps)) and realpower<((dynlevel*10+(triggerpowermode2*-10))+(52*ConeAmps)+(52*CtwoAmps))) {
     digitalWrite(relayPin1,HIGH);
     digitalWrite(relayPin2,LOW);
     chargerone=0;
     chargertwo=1;
   } 
   if (realpower>=((dynlevel*10+(triggerpowermode2*-10))+(52*ConeAmps)+(52*CtwoAmps))) {
     digitalWrite(relayPin1,LOW);
     chargerone=1;
     if (chargerlock==0) {
     digitalWrite(relayPin2,LOW);
...
if (typeofbatt==1 and inverter==1) {
  chargerlock=0;
   ...
   if (realpower<=((triggerpowermode1*10)+(52*ConeAmps))) {
     chargerone=0;
     chargertwo=0;
   } 
   if (realpower>((triggerpowermode1*10)+(52*ConeAmps)) and realpower<((triggerpowermode1*10)+(52*CtwoAmps))) {
     chargerone=1;
     chargertwo=0;
   }
    if (realpower>=((triggerpowermode1*10)+(52*CtwoAmps)) and realpower<((triggerpowermode1*10)+(52*ConeAmps)+(52*CtwoAmps))) {
     chargerone=0;
     chargertwo=1;
   } 
   if (realpower>=((triggerpowermode1*10)+(52*ConeAmps)+(52*CtwoAmps))) {
     chargerone=1;
     chargertwo=1;
     }
   } 
...
if (setpos==2){
 ...
 // set year
   currentStateCLK = digitalRead(CLK);
     ...
     if (counter<2020 or counter>2035) { counter=2020;
     }
     setyear=counter;
   } }
...
void startBlynk() {
  ...
  if (linkState == 0) {                               
    ...
    linkState = 1;                                   //Blynk startup done
  }
    ...
    if (chargerone==0 and chargertwo==0 and inverter==0){
         if (idleactive==0){
          ...
          idlediffmillis=millis()-idlestartmillis;
           if (idlediffmillis>300000){
            idlestatus=1;
           }
           if (idlediffmillis>600000){
            idlestatus=2;
           }
...
if (typeofbatt==1){
  ...
  SOCvoltage=SOCvoltage-(ConeAmps*0.056);
  corrindchg=7;
  }
  if (chargertwo==1) {         //correcting by charger voltage
  SOCvoltage=SOCvoltage-(CtwoAmps*0.056);
  corrindchg=7;
  }
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.056*0.20);  
  }
  }
  
  if (battvoltage>=54.70 and battvoltage<54.85){
  if (chargerone==1) {         
  SOCvoltage=SOCvoltage-(ConeAmps*0.045);
  corrindchg=6;
  }
  if (chargertwo==1) {         
  SOCvoltage=SOCvoltage-(CtwoAmps*0.045);
  corrindchg=6;
  }
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.045*0.20);  
  }
  }

   if (battvoltage>=54.40 and battvoltage<54.70){
  if (chargerone==1) {        
  SOCvoltage=SOCvoltage-(ConeAmps*0.035);
  corrindchg=5;
  }
  if (chargertwo==1) {       
  SOCvoltage=SOCvoltage-(CtwoAmps*0.035);
  corrindchg=5;
  }
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.035*0.20);  
  }
  }
  ...
  if (chargertwo==1) {      
  SOCvoltage=SOCvoltage-(CtwoAmps*0.029);
  corrindchg=4;
  }
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.029*0.20);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.024*0.20);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.021*0.20);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.017*0.20);  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
  SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.017*0.12);  
  }
}
...
if (corrindchg==4 and battSOC<62){
  battSOC=62;
}
if (corrindchg==5 and battSOC<81){
  battSOC=81;
}
if (corrindchg==6 and battSOC<88){
  battSOC=88;
}
if (corrindchg==7 and battSOC<91){
  battSOC=91;
}
