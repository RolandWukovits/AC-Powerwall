...
const int AnumReadings=5;          // how many values are part of MA
byte firstnbrs=0;
...
unsigned long Aloopinterval=350;         // interval of loop in mSec  
float watts;
byte Acounter=0;

#define Battswitch 14                   // enter your Switch Digital Pin
byte Battmode;
...
long battcapacity=12500;      //enter your battery capacity in Wh (max. 65300), you want to flash on EEPROM first time 
...
void setup(){
  ...
  if (EEPROMvalue[0]==255 or EEPROMvalue[0]==0) {
   ...
   extractor=cycles;
    ...
    if (coulcorrbyte>=125 or coulcorrbyte<=75){
       coulcorrarm=1;
       coulcorr=1.00;
    }
    else{
...
if (ExtWDT==1){
 ...
 // Wait 5 second
  ...
  battCapFact=(0.933+(battcapacity/50)*0.0001);
}
...
if (battSOC>=99 or Maxlock==1 or SOCCoul>maximumSOC) {
   digitalWrite(relayPin1,HIGH);
   digitalWrite(relayPin2,HIGH);
   chargerone=0;
   chargertwo=0;
...
if (millis()>=(cyclestartmillis+relaycycletime)) {
  ...
  if (coulstart==0){
    SOCCoul=battSOC;
    coulstart=1;
    CoulombPointer=(battcapacity/100)*SOCCoul;
    coulcorrstart=battSOC;
  }
 
// upper limit for charging
if (SOCCoul<=maximumSOC and Maxlock==0) {
...
void Mode11(){
  ...
  if (setpos==11){
  lcd.setCursor(12,3);
  if (setcoulcorrbyte==126 or setcoulcorrbyte==74){
    lcd.print("Rst "); 
  }
  else{    
    lcd.print(setcoulcorrbyte/100);
...
if (inverter==1 and mode<9){
  ...
  for (aquisitioncounter=0;aquisitioncounter<=141;aquisitioncounter++){
     
   readCurrent=ads.readADC_SingleEnded(3);  
...
if (diffCurrent<1){
  ...
  inverterwattsB=((invcurrsens*diffCurrent*230)/65536);  // calculate inverter watts
    inverterwattsB=inverterwattsB*1.08;   // correcting for lower MA result
      
  if (inverterwattsB>inverterlimitW){
    inverterwattsB=inverterlimitW;                              // limit for SUN2000GITL
  }

// Moving average
   totalW = totalW-Wreadings[WreadIndex];
   Wreadings[WreadIndex]=inverterwattsB;
   totalW = totalW+Wreadings[WreadIndex];
   
   WreadIndex++;

   if (WreadIndex>=numReadingsW){
     WreadIndex = 0;
     firstnbrs=1;
   }

   if (firstnbrs==0){
    inverterwatts=inverterwattsB/1.08;
   }
   else{
    inverterwatts = totalW/numReadingsW;
   }

  // adding DC side watts calculation
  invwattsbyshunt=(current*battvoltage)-70;           // 70W stanbby consumption
  inverterwatts=(inverterwatts+invwattsbyshunt)/2; 

  diffinverterwatts=inverterwatts-oldinverterwatts;
  if (diffinverterwatts<0){ 
  diffinverterwatts=diffinverterwatts*-1;
  }
  if (diffinverterwatts>80){
  SOCfreeze=SOCfreeze+16;  
  } 
  if (diffinverterwatts>130){
...
void getDischargeData(){
 ...
 if(diffSOC>82){
  ...
  watthoursTot=watthoursTot*1.01;                 // correct for missing 2% due to safety on bottom of curve
  watthoursTot=round(watthoursTot);
  AhoursTot=(ActAh/diffSOC)*100;
  AhoursTot=AhoursTot*1.02;                       // correct for missing 3% due to safety on bottom of curve
...
void Coulombmeter(){
 ...
 if (millis()>=(coulombstartmillis+wattsinterval)){
  ...
  if (coulstart==1){
     ...
     if (CoulombPointer>battcapacity){
        CoulombPointer=battcapacity;
     }
     SOCCoul=(CoulombPointer/battcapacity)*100;
  }   
...
if (chargerone==0 and chargertwo==0){
 if (battSOC<=15 and couldelay==0 and calistop==0){
...
void getCurrent(){
 ...
 if (Acounter>=7) {
   ...
   if (Afirstnbrs==0){
    ...
    current=current/10;
    current=current+0.3;                     // current offset correction
    if (current>0){
      current=current*coulcorr;                    // current calibration for discharge
    }
    // if (current<=0){
    //   current=current*1.1                       // current calibration for charge
    // }
...
void getVoltage(){
 ...
 diffbattvoltage=battvoltage-oldbattvoltage;
  ...
  if (diffbattvoltage>0.02 and battvoltage<54.9){
...
if (inverter==1 and blockdischargetest==0) {
...
if (typeofbatt==1){
 ...
 if (current<=0){                       //charging
  if (battvoltage>=54.85){
    SOCvoltage=SOCvoltage+(current*(0.055/battCapFact));
    corrindchg=8;
   }  
   
  if (battvoltage>=54.74 and battvoltage<54.85){
    SOCvoltage=SOCvoltage+(current*(0.044/battCapFact));
    corrindchg=7;           
    }  

  if (battvoltage>=54.60 and battvoltage<54.74){
    SOCvoltage=SOCvoltage+(current*(0.036/battCapFact));
    corrindchg=6;
    }  

  if (battvoltage>=54.39 and battvoltage<54.60){
    SOCvoltage=SOCvoltage+(current*(0.030/battCapFact));
    corrindchg=5;
   }  

   if (battvoltage>=53.85 and battvoltage<54.39){
    SOCvoltage=SOCvoltage+(current*(0.024/battCapFact));        // you might have to change current corrections according
    corrindchg=4;
   }  

  if (battvoltage>=53.10 and battvoltage<53.85){
    SOCvoltage=SOCvoltage+(current*(0.022/battCapFact));        // you might have to change current corrections according
   }  

  if (battvoltage<53.10) {  
     SOCvoltage=SOCvoltage+(current*(0.019/battCapFact));        // you might have to change current corrections according
   }  
 }

 if (current>(((ConeAmps+CtwoAmps)*-1)/2)){
  corrindchg++;
   if (corrindchg>8){
    corrindchg=8;
   }
...
if (inverter==1) {         //correcting by inverter voltage by Current module
 ...
 if (typeofbatt==1){
  ...
  SOCvoltage=SOCvoltage+(current*0.023);     // add by current
...
if (corrindchg==4 and battSOC<42){
  battSOC=42;
}
if (corrindchg==5 and battSOC<65){
  battSOC=65;
}
if (corrindchg==6 and battSOC<79){
  battSOC=79;
}
if (corrindchg==7 and battSOC<86){
  battSOC=86;
}
