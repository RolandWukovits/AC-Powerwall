...
unsigned long shortloopinterval=1400;         // start certain modes every 1200 milliseconds
unsigned long longloopinterval=2400;         // start certain modes every 2400 milliseconds
float averagevolt;
float holdvolt;
int voltcounter=1;
float SOCvoltage;
...
unsigned long currentinterval=20000;   // interval of current aquiration when inverting
...
/*
if (mode<9 and wifidis==0){   //keeps Blynk connection alive
  if (linkState==1){
   Blynk.run();
   wdt_reset(); 
  }
...
if (typeofbatt==1){
 ...
 if (battvoltage>=54.60 and battvoltage<54.74){
  ...
  if (chargerone==1) {        
  SOCvoltage=SOCvoltage-(ConeAmps*(0.030/battCapFact));
  corrindchg=7;
  }
  if (chargertwo==1) {       
  ...
  if (chargerone==1) {       
  SOCvoltage=SOCvoltage-(ConeAmps*(0.027/battCapFact));
  corrindchg=6;
  }
  if (chargertwo==1) {      
  ...
  if (chargerone==1) {     
  SOCvoltage=SOCvoltage-(ConeAmps*(0.022/battCapFact));
  corrindchg=4;
  }
  if (chargertwo==1) {     
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *(0.022/battCapFact)*0.16);
    corrindchg=3;  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *(0.020/battCapFact)*0.14);
    corrindchg=2;  
  }
  }
