...
// This is final-software for my AC-powerwall controller
// Version 1.01.04, 02.12.2020
...
byte relayAct3=0;
byte relayAct4=0;
int relaycycletime=190;    // set for the charger activation cycle time in loops
...
if (modecounter==1) {  //mode readout 1 time a second
  mode=1;
...
ledcounter++;
 ...
 if (mode==1 and loopcounter>=55000){
  Mode1();
  loopcounter=0;
  wdt_reset();
  }

 if (mode==2 and loopcounter>=55000){
  Mode2();
  loopcounter=0;
  wdt_reset();
 }
 
 if (mode==3 and loopcounter>=100000){
  Mode3();
  loopcounter=0;
  wdt_reset();
  }
  
 if (mode==4 and loopcounter>=55000){
  Mode4();
  loopcounter=0;
  wdt_reset();
  }
 ...
 if (currentcounter==1 and ledcounter<=80){
  Currentaquisition();
 }
...
if (pagelock==0){
  ...
  lcd.print("Inv ON:   h ");
  lcd.setCursor(8,1);
  if (setNormInvOn<10){ lcd.print("0");} 
  lcd.print(setNormInvOn);
  lcd.setCursor(12,1);
  lcd.print("OFF:   h ");
  lcd.setCursor(17,1);
  if (setNormInvOff<10){ lcd.print("0");} 
...
void Mode10(){
 ...
 if (pagelock==0){
  ...
  if (setpos==6){
  lcd.setCursor(11,3);
  lcd.print(setfineadjustmentbyte-20,0);
  if(setfineadjustmentbyte>10 and setfineadjustmentbyte<19){ lcd.print(" ");}
  if(setfineadjustmentbyte>=20 and setfineadjustmentbyte<30){ lcd.print("  ");}
  lcd.setCursor(15,3);
  lcd.print("<    ");  
  }
...
diffCurrent=diffCurrent*2.85;                   //current multiplier
...
void getVoltage(){
 ...
 diffbattvoltage=battvoltage-oldbattvoltage;
  ...
  if (diffbattvoltage>0.03 and battvoltage<55.1){
  SOCfreeze=SOCfreeze+6;  
  }
...
if (typeofbatt==1){
  ...
  if (chargerone==1) {         
  SOCvoltage=SOCvoltage-(ConeAmps*0.043);
  corrindchg=8;
  }
  if (chargertwo==1) {         
 ...
 if (battvoltage>=54.60 and battvoltage<54.74){
  if (chargerone==1) {        
  SOCvoltage=SOCvoltage-(ConeAmps*0.034);
  corrindchg=7;
  }
  if (chargertwo==1) {       
  ...
  if (chargerone==1) {       
  SOCvoltage=SOCvoltage-(ConeAmps*0.027);
  corrindchg=5;
  }
  if (chargertwo==1) {      
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.026*0.18);
   corrindchg=4;  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.022*0.16);
    corrindchg=3;  
  }
  }
  ...
  if (chargerone==1 and chargertwo==1){
   SOCvoltage=SOCvoltage+((ConeAmps+CtwoAmps) *0.020*0.14);
    corrindchg=2;  
  }
  }
  ...
  if (chargerone==1) {      
  SOCvoltage=SOCvoltage-(ConeAmps*0.016);
  corrindchg=1;
  }
  if (chargertwo==1) {     
